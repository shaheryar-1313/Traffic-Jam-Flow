...
namespace SupersonicWisdomSDK
{
    internal static class SwConfigUtils
    {
        ...
        public static List<KeyValuePair<string, object>> GetValueWithPrefix(this Dictionary<string, object> self, string prefix)
        {
            var pairs = new List<KeyValuePair<string, object>>();

            if (self != null)
            {
                foreach (var dictPair in self)
                {
                    if (dictPair.Key.StartsWith(prefix))
                    {
                        pairs.Add(dictPair);
                    }
                }
        ...
        internal static Dictionary<string, object> ResolveDeepLinkConfig([NotNull] Dictionary<string, string> deepLinkParams)
        {
            ...
            configFromDeepLink.SwMerge(true, SwUtils.JsonHandler.DeserializeObject<Dictionary<string, object>>(partialConfigDeepLinkRaw));
