...
namespace SupersonicWisdomSDK
{
    internal class SwSystemUtils
    {
        ...
        /// <summary>
        /// Determines if versionB is a newer version than versionA.
        /// Version strings are expected to be in a dot-separated format (e.g., "1.2.3").
        /// If either of the version strings is null or empty, the method returns false and logs an error.
        /// </summary>
        /// <param name="versionA">The current version string.</param>
        /// <param name="versionB">The version string to compare against the current version.</param>
        /// <returns>
        /// True if versionB is newer than versionA; otherwise, false.
        /// If any version string is invalid, false is returned and an error is logged.
        /// </returns>
        internal bool IsBVersionNewer(string versionA, string versionB)
        {
            var versionAInts = ConvertVersionToIntArray(versionA);
            var versionBInts = ConvertVersionToIntArray(versionB);
            
            if (versionAInts == null || versionBInts == null)
            {
                return false;
            }
            
            var maxLength = Math.Max(versionAInts.Length, versionBInts.Length);
            
            for (var i = 0; i < maxLength; i++)
            {
                var partA = i < versionAInts.Length ? versionAInts[i] : 0;
                var partB = i < versionBInts.Length ? versionBInts[i] : 0;
                
                if (partB > partA) return true;
                if (partB < partA) return false;
            }
            
            return false;
            
            int[] ConvertVersionToIntArray(string version)
            {
                if (!string.IsNullOrEmpty(version))
                {
                    return version.Split('.')
                        .Select(part => int.TryParse(part, out var val) ? val : 0)
                        .ToArray();
                }
                
                SwInfra.Logger.LogError(EWisdomLogType.AppUpdate, "Invalid version string: null or empty");
                return null;
