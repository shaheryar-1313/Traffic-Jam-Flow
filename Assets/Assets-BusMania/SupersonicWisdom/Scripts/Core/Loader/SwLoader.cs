    ...
    internal class SwLoader : ISwLoader
    {
        ...
        private void NotifyListeners(bool isVisible)
        {
            ...
            delegates?.SwForEach(listener =>
            {
                ...
                {
                    SwInfra.Logger.LogException(e, EWisdomLogType.BlockingLoader, $"Caught exception during listeners invocation: {nameof(OnBlockingLoaderVisibilityChangedEvent)}?.Invoke({isVisible});");
                }
            });
        }
