...
namespace SupersonicWisdomSDK
{
    internal class SwNativeRequestManager
    {
        ...
        public void SendRequest(string url, string headers, string body, ISwNativeRequestListener listener, int connectionTimeout, int readTimeout, int cap)
        {
            ...
            var requestJsonString = SwUtils.JsonHandler.SerializeObject(new SwRemoteRequest
            {
                key = key,
                url = url,
                headers = headers,
        ...
        private void OnResponse(string response)
        {
            ...
            {
                var swWebResponse = SwUtils.JsonHandler.DeserializeObject<SwWebResponse>(response, new JsonSerializerSettings
                {
        ...
        {
            if (code >= (int)SwNetworkConstants.HttpStatusCode.Ok && code <= 299)
            {
