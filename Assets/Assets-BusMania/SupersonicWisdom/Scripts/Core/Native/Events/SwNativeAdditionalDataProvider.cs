    ...
    internal class SwNativeAdditionalDataProvider
    {
        ...
        public string GetAdditionalDataJson()
        {
            ...
            foreach (var provider in _trackerDataProviders)
            {
                ...
                {
                    SwInfra.Logger.LogException(e, EWisdomLogType.Analytics, $"An error occured while trying to merge {provider}");
                }
            }

            return SwUtils.JsonHandler.SerializeObject(extraData);
        }
