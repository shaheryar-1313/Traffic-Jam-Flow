...
namespace SupersonicWisdomSDK.Editor
{
    internal class SwNativeEditorApi : SwNativeApi
    {
        ...
        {
            var request = SwUtils.JsonHandler.DeserializeObject<SwRemoteRequest>(requestJsonString);
        ...
        private IEnumerator SendRequestRoutine(string key, string url, string headers, string body)
        {
            ...
            if (response.DidSucceed)
            {
                ...
                {
                    _webResponseCallback?.Invoke(SwUtils.JsonHandler.SerializeObject(response));
                }
                else
                {
