...
namespace SupersonicWisdomSDK
{
    ...
    public class SwSettings : ScriptableObject, ISwSettings
    {
        ...
        [SerializeField] public bool enableNoInternetOnAppOpen;
        [SerializeField] public int graceNoInternetOnAppOpen;
...
#pragma warning restore 0649
        ...
        public override string ToString()
        {
            ...
            sb.AppendLine($"iosChinaBuildEnabled:{iosChinaBuildEnabled.SwToString().ToLower()}");
            
            sb.AppendLine($"enableUnityAnalyticsIntegration:{enableUnityAnalyticsIntegration.SwToString().ToLower()}");
            sb.AppendLine($"isTimeBased:{isTimeBased.SwToString().ToLower()}");
            sb.AppendLine($"enableManagedInterstitialAds:{enableManagedInterstitialAds.SwToString().ToLower()}");
            sb.AppendLine($"enableManagedBannerAds:{enableManagedBannerAds.SwToString().ToLower()}");
            sb.AppendLine($"enableManagedRateUsPopup:{enableManagedRateUsPopup.SwToString().ToLower()}");
        ...
        public virtual Dictionary<string, object> GetValues()
        {
            ...
            //This implementation needs to be moved to SwStage30Settings in the future when we will refactor Editor with Stages
            //The reason we have key duplications because the Editor is built without stages and these key suppose to be in stage 30 - will be merged when there will be Stage30SwSettings
            const string IS_TIME_BASED_KEY = "swAdsIsTimeBased"; // Duplicate key - SwStage30AdStrategyLocalConfig.IS_TIME_BASED_KEY
            const string LOADING_GAME_OFFLINE_COUNTDOWN_LEGACY = "swNoInternetLoadingGameOfflineCountdown"; //Duplicate key - SwStage30NoInternetLocalConfig.LOADING_GAME_OFFLINE_COUNTDOWN_LEGACY
            
            var values = new Dictionary<string, object>
            {
                { IS_TIME_BASED_KEY, isTimeBased },
            };
            
            if (enableNoInternetOnAppOpen)
            {
                values.SwAddOrReplace(LOADING_GAME_OFFLINE_COUNTDOWN_LEGACY , graceNoInternetOnAppOpen);
            }
