...
namespace SupersonicWisdomSDK
{
    internal class SwBlockingApiHandler
    {
        ...
        private IEnumerator NotifyLevelContinued(ESwLevelType? levelType, long level, string customString)
        {
            var waitForBlockerClose = Lock($"NotifyLevelContinued({level}, \"{customString}\")");
            // if levelType is null, we assume it's a regular level
            var swLevelType = levelType ?? ESwLevelType.Regular;
            
            SwInfra.Logger.Log(EWisdomLogType.IntegrationTool, $"{swLevelType} {level}");

            _swGameStateSystem.ProcessLevelContinued(swLevelType, level, customString);

            foreach (var middleware in _middlewares)
            {
                yield return middleware.ProcessLevelContinued();
            }

            yield return Unlock(waitForBlockerClose);
        }
        
        public void NotifyLevelContinued(ESwLevelType? levelType, long level, Action action, string customString)
        {
            SwInfra.CoroutineService.StartCoroutineWithCallback(() => NotifyLevelContinued(levelType, level, customString), action);
        }
        ...
        public void NotifyUiTriggerPoint(string triggerPoint, Action action)
        {
            SwInfra.CoroutineService.StartCoroutineWithCallback(() => NotifyUiTriggerPoint(triggerPoint), action);
        }
        
        public IEnumerator NotifyUiTriggerPoint(string triggerPoint)
        {
            SwInfra.Logger.Log(EWisdomLogType.IntegrationTool);
            
            var waitForBlockerClose = Lock($"NotifyUiTriggerPoint({triggerPoint})");
            
            foreach (var middleware in _middlewares)
            {
                yield return middleware.ProcessUiTriggerPoint(triggerPoint);
            }
