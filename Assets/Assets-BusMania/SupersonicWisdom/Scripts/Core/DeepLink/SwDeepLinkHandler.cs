...
namespace SupersonicWisdomSDK
{
    internal class SwDeepLinkHandler
    {
        ...
        private IEnumerator VerifyDeepLinkParams(Dictionary<string, string> deepLinkParams)
        {
            ...
            if (isVerified)
            {
                SwInfra.Logger.Log(EWisdomLogType.DeepLink, SwUtils.JsonHandler.SerializeObject(deepLinkParams));
            }
            else
            {
                SwInfra.Logger.LogError(EWisdomLogType.DeepLink,  SwUtils.JsonHandler.SerializeObject(deepLinkParams));
        ...
        private IEnumerator TryIsAdminUser(Dictionary<string, string> deepLinkParams, bool isAdminVerificationRequired, Action<bool> isVerifiedResult)
        {
            ...
            if (!string.IsNullOrEmpty(authorizationHeaderValue))
            {
                ...
                if (response.DidSucceed)
                {
                    try
                    {
                        var platformUser = SwUtils.JsonHandler.DeserializeObject<SwPlatformUser>(response.Text);
                    ...
                    {
                        SwInfra.Logger.LogException(e, EWisdomLogType.DeepLink,
                            $"Cannot deserialize SwPlatformUser from response={response.Text}");
                    }
                }
            }
        }
