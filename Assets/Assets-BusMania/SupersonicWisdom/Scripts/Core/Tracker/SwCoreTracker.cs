...
namespace SupersonicWisdomSDK
{
    internal class SwCoreTracker : ISwTracker
    {
        ...
        public void TrackEventWithParams(string eventName, Dictionary<string, object> customs = null, ClientCategory? clientCategory = null)
        {
            ...
            var extraData = GetEventExtraDataFromListeners(eventName);
            ...
            var customsJson = SwUtils.JsonHandler.SerializeObject(customs, settings);
            var extraJson = SwUtils.JsonHandler.SerializeObject(GetEventDetailsExtra(), settings);
        ...
        public void TrackEventWithParams(string eventName, Dictionary<string, object> customsDictionary, ClientCategory? clientCategory = null, params string[] customs)
        {
            ...
            var extraJson = SwUtils.JsonHandler.SerializeObject(GetEventDetailsExtra());
        ...
        private SwJsonDictionary GetEventExtraDataFromListeners(string eventName)
        {
            ...
            foreach (var listener in _trackerListeners)
            {
                try
                {
                    var (listenerData, keysToEncrypt) = listener.ConditionallyAddExtraDataToTrackEvent(_coreUserData, eventName);
                ...
                {
                    SwInfra.Logger.LogException(e, EWisdomLogType.Analytics, $"An error occured while trying to merge {listener}");
                }
            }
        ...
        private static IEnumerator SendUrlCoroutine(string url)
        {
            ...
            using (var webRequest = UnityWebRequest.Get(url))
            {
                ...
                if (code == 0 || code >= (int)SwNetworkConstants.HttpStatusCode.BadRequest)
                {
