...
namespace SupersonicWisdomSDK
{
    public static class Stage10ApiExtension
    {
        ...
        /// <summary>
        /// Notifies that a level has continued.
        /// </summary>
        /// <param name="levelType">The type of the level.</param>
        /// <param name="level">The level number.</param>
        /// <param name="action">The action to be performed after notifying the level continued.</param>
        /// <param name="customString">Optional custom string parameter.</param>
        [PublicAPI]
        public static void NotifyLevelContinued(this SwApi self, ESwLevelType levelType, long level, Action action,
            string customString = "")
        {
            if (!Container.Settings.isTimeBased)
            {
                self.RunWithContainerAndReadyOrThrow(() => Container.BlockingApiHandler.NotifyLevelContinued(levelType, level, action, customString),
                    nameof(NotifyLevelContinued));
            }
            else
            {
                self.HandleUnsupportedLevelBasedApiCall(nameof(NotifyLevelContinued));
            }
        }
        ...
        /// <summary>
        /// This method is used to notify a game event occured - the SDK might perform async action based on configuration.
        /// </summary>
        /// <param name="triggers">Sequence of game events in the chronological order (first-event, ...., last-event)</param>
        [PublicAPI]
        public static IEnumerator NotifyTrigger(this SwApi self, params string[] triggers)
        {
            return self.RunAsyncWithContainerOrThrow(() => Container.TacSystem.FireTriggers(triggers), nameof(NotifyTrigger));
        }
        ...
        /// <param name="triggers">Sequence of game events in the chronological order (first-event, ...., last-event)</param>
        [PublicAPI]
        public static void NotifyTrigger(this SwApi self, Action callback, params string[] triggers)
        {
            self.RunWithContainerAndReadyOrThrow(() =>
                {
                    SwInfra.CoroutineService.StartCoroutineWithCallback
                        (() => Container.TacSystem.FireTriggers(triggers), callback);
                }, 
                nameof(NotifyTrigger));
        }
