...
namespace SupersonicWisdomSDK
{
    internal class SwAliveStatusSender : SwBaseAnalyticsManager, ISwCoreConfigListener, ISwGameStateSystemListener, ISwLocalConfigProvider
    {
        ...
        {
            _aliveEventIntervalsRawConfig = configAccessor.GetValue(SwAliveSenderLocalConfig.ALIVE_EVENT_INTERVALS_CONFIG_KEY, SwAliveSenderLocalConfig.ALIVE_EVENT_INTERVALS_CONFIG_VALUE.ToString());
            ...
            SwInfra.Logger.Log(EWisdomLogType.Alive, $"Configuration \"{_aliveEventIntervalsRawConfig}\" parse to {SwUtils.JsonHandler.SerializeObject(_configDict)}");
        ...
        private void OverrideCustomCustomConfig()
        {
            try
            {
                ...
                var stateConfigDict = SwUtils.JsonHandler.DeserializeObject<Dictionary<SwSystemState.EGameState, int>>(_aliveEventIntervalsRawConfig, settings);

                _configDict.SwMerge(true, stateConfigDict);
            }
            catch (Exception e)
            {
                SwInfra.Logger.LogException(e, EWisdomLogType.Alive, $"Failed {nameof(OverrideCustomCustomConfig)}");
            }
        }
