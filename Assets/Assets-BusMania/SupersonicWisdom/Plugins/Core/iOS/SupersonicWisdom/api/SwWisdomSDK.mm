...
extern "C" {
    ...
    void openAppStorePageWithAppId(const char* appId) {
        NSString* appIdString = [NSString stringWithUTF8String:appId];
        [SwWisdomSDK openAppStorePageWithAppId:appIdString];
    }
    ...
    float getScreenWidth() {
        ...
        CGFloat width = screenBounds.size.height * screenScale;
...
+ (void)openAppStorePageWithAppId:(NSString *)appStoreId {
    if(isInitialized()){
        [sharedInstance openAppStorePageWithAppId:appStoreId];
    }
}
